...
export function decide(
  ...
  if (state.lastAutomaticTurn !== undefined && state.turn - state.lastAutomaticTurn < options.cooldownTurns) {
    return { allowed: false, kind: "none", reason: "cooldown" };
  }
  ...
  const sample = stableSample(`${options.sessionKey}:${state.turn}:${text}`);
  if (sample >= threshold) return { allowed: false, kind: "none", reason: "probability_gate" };
