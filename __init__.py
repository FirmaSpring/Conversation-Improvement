...
def _pre_llm_call(session_id: str = "", user_message: str = "", is_first_turn: bool = False, **_: object):
    ...
    common = (
        ...
        "GLOBAL ENFORCEMENT: NEVER call conversation_image_generate as the first image step. Before any image expression or image request, first call conversation_image_search with query='' and limit=20 to discover actual titles/filenames under C:\\Users\\ChainBox\\AppData\\Local\\hermes\\conversation-improvement\\library\\media (or active $HERMES_HOME equivalent). The result returns available_titles and paths, so never guess a keyword. "
        "Choose a contextually fitting title from available_titles; if more filtering is needed, call conversation_image_search again with that exact title. "
        "If a title matches the conversation, deliver that returned path directly as MEDIA: and STOP; do not call generation. Only if title search finds no contextually suitable image may you call conversation_image_generate. "
